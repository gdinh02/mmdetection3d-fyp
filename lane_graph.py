...
def infer_lane_boundaries(lane_fits, cfg=None):
    ...
    for i in range(len(lane_fits)):
        for j in range(i + 1, len(lane_fits)):
            ...
            print(
                f"S{a['stream_id']} vs S{b['stream_id']}: "
                f"overlap={overlap:.2f}, "
                f"x_gap={np.median(np.abs(delta)):.2f}, "
                f"width={np.median(np.abs(widths)):.2f}"
            )
            if abs(median_delta) < 1e-6:
                print("Median delta too small")
                continue
            if np.mean(np.sign(delta) == np.sign(median_delta)) < 0.9:
                print("Not enough consistent signs")
                continue
            ...
            slope = 0.5 * (
                lane_polynomial_slope(left["coefficients"], z)
                + lane_polynomial_slope(right["coefficients"], z)
            )
            widths = (x_right - x_left) / np.sqrt(1.0 + slope**2)
            median_width = float(np.median(widths))
            if not (cfg.min_lane_width <= median_width <= cfg.max_lane_width):
                print("Incorrect lane width")
                continue
            ...
            boundaries.append(
                {
                    ...
                    "lane_width_std": float(np.std(widths)),
                }
            )
